...
class WebMeridian_Megamenu_Block_Navigation extends Smartwave_Megamenu_Block_Navigation
{
    ...
    public function drawMenuItem($children, $level = 1, $type, $width, $mode = 'dt')
    {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                $_product_count ='('.$collection->count().')'.
                $active = '';
                ...
                if ($level == 1) {
                    ...
                    $html .= '<li class="'.$class.' '.$active.' ">';
          					...
          					//version 1.0.2 end
                    $html.= '<a class="level' . $level . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name .$label.'  '. $_product_count.'</span></a>';
                } else {
                    $html .= '<li class="'.$class.' '.$active.'">';
                    $html .= '<a class="level' . $level . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name .$label.'  '. $_product_count.'</span></a>';
                }
